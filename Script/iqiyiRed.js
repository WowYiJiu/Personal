 ...
 * 脚本作者：WowYiJiu
 * 更新日期：2024-02-08
 * 脚本参考自 @shengetui：https://github.com/shengetui
 * 脚本说明：进入爱奇艺云包场界面抓https://act.vip.iqiyi.com/supermk/seckill/query/activity/list的url，将list?后面的请求参数以一直到cs=1复制替换脚本中seckill?请求参数覆盖到cs=1，
   ${timestamp}无需替换，code值是随每场抢购变化,抓到的list包里就对应每场code值
 * Unix时间戳网站：https://tool.chinaz.com/tools/unixtime.aspx 
*/

const $ = new Env('爱奇艺云包场');
...
async function main() {
    ...
    let opt = {
        url: `https://act.vip.iqiyi.com/supermk/seckill/action/seckill?page=30&abt=&u=${u}&de=${de}&deviceIdType=1003&platform=${platform}&version=15.2.0&deviceId=${deviceId}&qyid=${qyid}&P00001=${P00001}&dfp=${dfp}&lang=zh_CN&app_lm=cn&ptid=${ptid}&agentType=12&fv=${fv}&source=${source}&cs=1&_=${timestamp}&code=${code}`,
        method: "GET",       
        headers: {
            ...
            'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 IqiyiApp/iqiyi IqiyiVersion/15.2.0  IqiyiPlatform/2_22_221 WebVersion/QYWebContainer QYStyleModel/(light)',
            'Refere': 'https://vip.iqiyi.com/',

 ...
 * @Last Modified by: WowYiJiu
 * @Last Modified time: 2024-2-25
 * sendNotify 推送通知功能
 * @param text 通知头
 * @param desp 通知体
...
async function sendNotify(
  ...
  // 根据标题跳过一些消息推送，有提示失效的信息仍会推送，环境变量：SKIP_PUSH_TITLE 用&分隔
  let skipTitle = [];
  skipTitle = process.env.SKIP_PUSH_TITLE.split('&');;
  if (skipTitle.length > 0) {
    for (var title in skipTitle) {
      if (text.indexOf(skipTitle[title]) != -1 && desp.indexOf('失效') === -1 && desp.indexOf('错误') === -1) {
          console.log("检测到标题存在屏蔽推送的关键字(" + skipTitle[title] + ")，将跳过推送...");
          return;
      }
    }
  }
